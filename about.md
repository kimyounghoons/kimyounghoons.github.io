---
layout: page
title: About
description: "안드로이드 개발자 김영훈의 개발 블로그 소개입니다."
permalink: /about/
share: true
---

# 안녕하세요, 안드로이드 개발자 김영훈입니다

이 블로그는 제가 안드로이드 앱을 만들면서 직접 부딪힌 문제와 그 해결 과정을 기록하는 공간입니다.
검색해도 한국어 자료가 없어 한참 헤맸던 내용, 공식 문서만으로는 감이 안 잡혔던 API,
실제 서비스에 적용해 보고 나서야 알게 된 것들을 정리해 두고 있습니다.

## 다루는 주제

- **Android** — Activity/Fragment 생명주기, RecyclerView, WorkManager, Firebase 연동, 빌드 설정(Gradle, buildTypes, resConfig) 등 실무에서 반복해서 만나는 주제
- **Jetpack Compose** — Recomposition 디버깅, Compose 컴파일러 리포트 분석, Pager·Indicator 같은 UI 구성 요소
- **Kotlin** — 코루틴, 함수형 프로그래밍, 언어 기능 활용 패턴
- **개발 환경** — Android Studio, Git, macOS 세팅과 생산성 도구

모든 글은 직접 작성했고, 예제 코드는 실제로 빌드해 동작을 확인한 뒤 올립니다.

## 글을 쓰는 기준

1. **재현 가능할 것** — 코드 조각만 던지지 않고, 어떤 환경(버전, 의존성)에서 동작하는지 함께 남깁니다.
2. **왜 그런지 설명할 것** — 해결책뿐 아니라 원인을 같이 적습니다. 다음에 비슷한 문제를 만났을 때 응용할 수 있어야 하니까요.
3. **틀리면 고칠 것** — 이후에 더 나은 방법을 알게 되면 기존 글을 수정합니다.

## 둘러보기

- [전체 글 보기](/posts/)
- [카테고리별 보기](/categories/)
- [태그별 보기](/tags/)

## 연락처

블로그 내용에 대한 질문, 오류 제보, 제안은 언제든 환영합니다.

- 이메일: [kimyh8496@gmail.com](mailto:kimyh8496@gmail.com)
- GitHub: [github.com/kimyounghoons](https://github.com/kimyounghoons)

관련 문서: [개인정보처리방침](/privacy/) · [문의하기](/contact/)
